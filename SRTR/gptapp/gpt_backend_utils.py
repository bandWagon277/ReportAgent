...
def get_prompt_path(output_type):
    ...
    base_path = os.getenv('PROMPT_BASE_PATH', r"C:/Users/18120/Desktop/OPENAIproj/")
...
def build_enhanced_agentB_messages(manifest: Dict[str, Any], 
                                 instruction_template_B: str | None) -> List[Dict[str, str]]:
    """
    Build enhanced Agent B messages with statistical data from manifest
    """
    import json
    
    # Enhanced system prompt for Agent B
    ...
    For each figure:
    1. Reference specific values from data_summary.key_stats
    2. Mention key data points and trends from data_summary.data_points  
    3. Add clinical implications based on the statistical findings
    4. Write 3-4 sentences with specific numbers and percentages
    
    For each table:
    1. Reference specific percentages and counts from data_summary
    2. Highlight the most significant findings with actual numbers
    3. Compare key categories and their clinical relevance
    
    Example good insight:
    "The age distribution shows a mean age of 52.4 years with 23.6% of patients in the 40-49 age group. 
    This older patient demographic (median 51.0 years) suggests increased monitoring requirements for 
    age-related complications. The slight right skew indicates a substantial elderly population requiring 
    ...
    # Process figures with data_summary
    if "figures" in manifest:
        for fig in manifest["figures"]:
            if "data_summary" in fig:
                print(fig["data_summary"])
    ...
    # Process tables with data_summary  
    ...
    # Build user message with statistical data
    user_prompt = f"""
Please create a comprehensive PDF report using the following manifest and detailed statistical data:

MANIFEST OVERVIEW:
{json.dumps({k: v for k, v in manifest.items() if k not in ['figures', 'tables']}, indent=2)}
...
Instructions:
- Use the data_summary for each figure/table to write specific, data-driven insights
...
- Place the analytical paragraph immediately after each figure/table in the PDF
- Use the statistical insights to create meaningful clinical interpretations

Generate the PDF composer code that uses asset_loader and table_loader functions (already provided in environment).
"""
...
Generate Python code that analyzes the data according to the user requirements.
"""
...
def execute_composer_code(py_file_path: str, manifest: Dict[str, Any], manifest_paths: Dict[str, Dict[str, str]]) -> Tuple[bytes, str]:
    ...
    def table_loader(table_id: str) -> str:
